...
// YP had to compile this file using:
// i386-elf-ld -m    elf_i386 -N -e main -Ttext 0 -o _lotterytest lotterytest.o
// ulib.o usys.o printf.o umalloc.o
// /opt/i386-elf/lib/gcc/i386-elf/15.2.0/libgcc.a
...
__attribute__((noreturn)) void exit_fast() { exit(); }
...
void execute_and_get_info(struct test_case *test, int *pids,
...
void count_schedules(struct test_case *test, int *pids,
                     struct processes_info *before,
                     struct processes_info *after) {
  test->total_actual_schedules = 0;
  for (int i = 0; i < test->num_children; ++i) {
    int before_index =
        find_index_of_pid(before->pids, before->num_processes, pids[i]);
    int after_index =
        find_index_of_pid(after->pids, after->num_processes, pids[i]);
    check(test, before_index >= 0 && after_index >= 0,
    ...
    if (before_index >= 0 && after_index >= 0) {
      ...
      test->actual_schedules[i] = after->times_scheduled[after_index] -
                                  before->times_scheduled[before_index];
      test->total_actual_schedules += test->actual_schedules[i];
    } else {
      test->actual_schedules[i] =
          -99999; // obviously bogus count that will fail checks later
    }
  }
}
...
void dump_test_timings(struct test_case *test) {
  ...
  for (int i = 0; i < test->num_children; ++i) {
    ...
    printf(1, "%d\t%d\t%d\t(assigned %d tickets; running %s)\n", i,
           test->expect_schedules_unscaled[i], test->actual_schedules[i],
           test->tickets[i], assigned_function);
  }
...
/* The comapre_schedules() code tries to do a Chi-squared test.
   ...
   Since we can't use floating point in xv6, calculations are done in fixed
   point.
*/
...
const long long chi_squared_thresholds[] = {
    FIXED_POINT_BASE / 100 * 1083, FIXED_POINT_BASE / 100 * 1382,
    FIXED_POINT_BASE / 100 * 1627, FIXED_POINT_BASE / 100 * 1847,
    FIXED_POINT_BASE / 100 * 2052, FIXED_POINT_BASE / 100 * 2246,
    FIXED_POINT_BASE / 100 * 2432, FIXED_POINT_BASE / 100 * 2612,
    FIXED_POINT_BASE / 100 * 2788, FIXED_POINT_BASE / 100 * 2959,
};
...
int compare_schedules_chi_squared(struct test_case *test) {
  ...
  /*
     compute the expected number of schedules for each child given the total
     number (in fixed point, so the rounding isn't so extreme.

     then, use this to compute the chi-squared test statistic:

      sum( (observed - expected)^2 / expected )

     then compare this to our table of thresholds on the chi-squared
     distribution for the appropriate degree of freedom

     as a special case, we simply ignore columns for which we expect 0 items,
     except that other columns are computed based on the portion of the actual
     total number of schedules.

     a better solution would be to use a statistical test that can handle this
     case, like Fisher's exact test.
  */
  long long delta = 0;
  int skipped = 0;
  for (int i = 0; i < test->num_children; ++i) {
    long long scaled_expected =
        (test->expect_schedules_unscaled[i] << FIXED_POINT_COUNT) /
        expect_schedules_total * test->total_actual_schedules;
#ifdef DEBUG
    printf(1, "scaled %d to %d for ratio %d:%d\n",
           test->expect_schedules_unscaled[i],
           (int)(scaled_expected >> FIXED_POINT_COUNT),
           (int)expect_schedules_total, test->total_actual_schedules);
#endif
    ...
    long long cur_delta =
        scaled_expected - (test->actual_schedules[i] << FIXED_POINT_COUNT);
#ifdef DEBUG
...
#endif
    // cur_delta >>= FIXED_POINT_COUNT; // skipped because cancelled out by
    // future shift back
#ifdef DEBUG
  ...
  printf(1, "%s test statistic %d (rounded)\n", test->name,
         (int)((delta + FIXED_POINT_BASE / 2) >> FIXED_POINT_COUNT));
#endif
  ...
  check(test,
        test->total_actual_schedules > (test->override_min_schedules == 0
                                            ? MIN_SCHEDULES
                                            : test->override_min_schedules),
...
void compare_schedules_naive(struct test_case *test) {
  ...
  int failed_any = 0;
  for (int i = 0; i < test->num_children; ++i) {
    long long scaled_expected = ((long long)test->expect_schedules_unscaled[i] *
                                 test->total_actual_schedules) /
                                expect_schedules_total;
    int max_expected = scaled_expected * 11 / 10 + 10;
    int min_expected = scaled_expected * 9 / 10 - 10;
    int in_range = (test->actual_schedules[i] >= min_expected &&
                    test->actual_schedules[i] <= max_expected);
    if (!in_range) {
      failed_any = 1;
    }
  }
  check(test, !failed_any,
        "schedule counts within +/- 10% or +/- 10 of expected");
  if (!failed_any) {
...
void run_test_case(struct test_case *test) {
  ...
  check(test,
        before.num_processes < NPROC && after.num_processes < NPROC &&
            before.num_processes > test->num_children &&
            after.num_processes > test->num_children,
  ...
  if (!compare_schedules_chi_squared(test)) {
    ...
    check(
        test, 1,
        "assuming schedule counts approximately right given chi-squared test");
  }
  printf(1, "%s: passed %d of %d\n", test->name,
         test->total_tests - test->errors, test->total_tests);
}
