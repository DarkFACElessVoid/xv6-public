...
// can change #undef USE_YIELD to #define USE_YIELD
#undef USE_YIELD
...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    printf(2,
           "usage: %s seconds tickets1 tickets2 ... ticketsN\n"
  ...
  for (int i = 0; i < num_children; ++i) {
    ...
    printf(1, "spawned child %d with %d tickets (pid %d)\n", i, tickets,
           active_pids[i]);
  }
  wait_for_ticket_counts(num_children, active_pids, tickets_for);
  report_processes_info();
  ...
  if (before.num_processes >= NPROC || after.num_processes >= NPROC) {
    printf(2, "getprocessesinfo's num_processes is greater than NPROC before "
              "parent slept\n");
    return 1;
  }
  if (before.num_processes < 0 || after.num_processes < 0) {
    printf(2, "getprocessesinfo's num_processes is negative -- not changed by "
              "syscall?\n");
    return 1;
  }
  ...
  for (int i = 0; i < num_children; ++i) {
    int before_index =
        find_index_of_pid(before.pids, before.num_processes, active_pids[i]);
    int after_index =
        find_index_of_pid(after.pids, after.num_processes, active_pids[i]);
    if (before_index == -1)
      printf(
          2,
          "child %d did not exist for getprocessesinfo before parent slept\n",
          i);
    if (after_index == -1)
      printf(2,
             "child %d did not exist for getprocessesinfo after parent slept\n",
             i);
    if (before_index == -1 || after_index == -1) {
      ...
      if (before.tickets[before_index] != tickets_for[i]) {
        printf(2,
               "child %d had wrong number of tickets in getprocessinfo before "
               "parent slept\n",
               i);
      }
      if (after.tickets[after_index] != tickets_for[i]) {
        printf(2,
               "child %d had wrong number of tickets in getprocessinfo after "
               "parent slept\n",
               i);
      }
      printf(1, "%d\t%d\n", tickets_for[i],
             after.times_scheduled[after_index] -
